...
-- update 구문 사용할 때는 천천히 where절 확인하기
...
-- 2. 아이디가 3인 사람의 메뉴를 '감바스'로 수정

UPDATE `order`
SET product_name = '감바스'
WHERE id = 3;

-- 3. 아이디가 5인 주문의 주문 일자를 '2022-01-05'로 수정
-- update 구문 사용시 DATE 타입은 값 입력시에 작은 따옴표로 감싸주기
UPDATE `order`
SET order_date = '2022-01-05'
WHERE id = 5;

-- 테이블 구조를 변경해 보기 : 컬럼 추가하는 방법
-- column은 추가해도 되고 제거해도 된다.
